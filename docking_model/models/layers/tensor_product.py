...
from docking_model.models.layers.normalization import get_norm_layer
from docking_model.models.layers.mlp import FCBlock
...
def maybe_import_oeq():
    try:
        if hasattr(torch, "library") and not hasattr(
            torch.library, "register_autocast"
        ):
            def register_autocast(*args, **kwargs):
                return None

            torch.library.register_autocast = register_autocast
...
def oeq_instructions_from_e3nn(instructions):
    converted = []
    for inst in instructions:
        if hasattr(inst, "i_in1"):
            converted.append(
                (
                    inst.i_in1,
                    inst.i_in2,
            ...
            if len(inst) < 6:
                raise ValueError(
                    "Unsupported instruction format for OpenEquivariance."
                )
            converted.append(tuple(inst[:6]))
    return converted


def tensor_tree_is_cuda(value):
    if isinstance(value, torch.Tensor):
        return value.is_cuda
    if isinstance(value, (list, tuple)):
        return all(tensor_tree_is_cuda(v) for v in value)
    return False
...
class TensorProductConvLayer(torch.nn.Module):
    def __init__(
        ...
        self.use_oeq_kernels = use_oeq_kernels
        if hidden_features is None:
            hidden_features = n_edge_features

        if self.use_oeq_kernels:
            if oeq is None:
                raise ImportError(
                    "openequivariance is not available but use_oeq_kernels=True."
                )
            if faster:
                rank_zero_info("OpenEquivariance enabled; ignoring FasterTensorProduct.")
            rank_zero_info("OpenEquivariance Tensor Product")
            tp_ref = o3.FullyConnectedTensorProduct(
                in_irreps, sh_irreps, out_irreps, shared_weights=False
            )
            instructions = oeq_instructions_from_e3nn(tp_ref.instructions)
            ...
            self.tp = oeq.TensorProduct(self.tp_problem, torch_op=True)
    ...
    def forward(
        ...
        if self.use_oeq_kernels and not (
            tensor_tree_is_cuda(node_attr)
            and tensor_tree_is_cuda(edge_attr)
            and tensor_tree_is_cuda(edge_sh)
        ):
            raise ValueError(
                "OpenEquivariance kernels require CUDA tensors; "
                "set use_oeq_kernels=False for CPU execution."
            )
