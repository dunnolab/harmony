...
def expansion(omega, eps, L=2000):  # the summation term only
    p = 0
    for l in range(L):
        p += (
            (2 * l + 1)
...
def score(exp, omega, eps, L=2000):  # score of density over SO(3)
    dSigma = 0
    for l in range(L):
...
if os.path.exists(".so3_omegas_array3.npy"):
    ...
    _exp_vals = np.asarray([expansion(_omegas_array, eps) for eps in _eps_array])
    _pdf_vals = np.asarray(
        [density(_exp, _omegas_array, marginal=True) for _exp in _exp_vals]
    )
    _cdf_vals = np.asarray([_pdf.cumsum() / X_N * np.pi for _pdf in _pdf_vals])
    _score_norms = np.asarray(
        [
            score(_exp_vals[i], _omegas_array, _eps_array[i])
