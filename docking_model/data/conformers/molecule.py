...
from docking_model.data.conformers.exceptions import time_limit
...
def generate_conformer(mol):
    ...
    if cid == -1:
        ...
        if cid == -1:
            raise ValueError("Embedding failed even with random coords.")
        
    try:
        props = AllChem.MMFFGetMoleculeProperties(mol, mmffVariant="MMFF94s")
        if props is None:
            # Fallback if MMFF not supported
            AllChem.UFFOptimizeMolecule(mol, confId=int(cid))
        else:
            AllChem.MMFFOptimizeMolecule(mol, confId=int(cid), mmffVariant="MMFF94s")
    except ValueError:
