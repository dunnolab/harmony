...
from docking_model.data.constants import allowable_features, periodic_table
from docking_model.data.feature.helpers import safe_index, get_fragment_index
...
def get_nearby_residue_mask(
    ...
    if nearby_residue_mask.sum() < min_residues:
        ...
        k = min(max(int(min_residues), 0), int(min_rec_dists.numel()))
        if k > 0:
            _, closest_residues = torch.topk(min_rec_dists, k=k, largest=False)
...
def get_binding_pocket_masks(
    ...
    min_keep = min(max(int(pocket_min_size), 1), int(ca_pos.shape[0]))
    nearby_residue_mask = get_nearby_residue_mask(
        ref_atom_pos,
        lig_pos,
        atom_rec_index,
    ...
    if nearby_residue_idxs.numel() == 0:
        pocket_center = ca_pos.mean(dim=0)
    else:
        pocket_center = torch.index_select(ca_pos, -2, nearby_residue_idxs).mean(
            dim=-2
        )
    pocket_res_mask = torch.linalg.norm(ca_pos - pocket_center, dim=-1) < pocket_buffer
    if pocket_res_mask.sum() < min_keep:
        pocket_res_dists = torch.linalg.norm(ca_pos - pocket_center, dim=-1)
        pocket_res_mask = torch.zeros_like(pocket_res_mask)
        _, closest_residues = torch.topk(pocket_res_dists, k=min_keep, largest=False)
        pocket_res_mask[closest_residues] = True
        if nearby_residue_idxs.numel() == 0:
            nearby_residue_idxs = closest_residues
