...
from docking_model.geometry.ops import axis_angle_to_matrix, matrix_to_axis_angle
...
def get_transformation_mask(pyg_data):
    ''' Compute which ligand edges correspond to rotatable bonds and which atoms move when each such bond is rotated '''
...
def get_fragment_index(edge_index, base_idx=None, edge_ptr=0):
    ''' For a directed bond graph, identify rotatable edges, their affected fragments, ring-related flags, and an ordering for applying rotations '''
    ...
    # Find atom indexes belonging to at least one simple cycle
    in_ring = set(sum(list(nx.simple_cycles(G.to_undirected())), []))

    # Iterative over bonds
    ...
    for edge_idx in range(0, edge_index.shape[1], 2):
        ...
        # If undirected graph without an edge is disconnected -> that bond is a candidate for rotatable one
        if not nx.is_connected(G2):
            # Retrieve and sort components so that fragments[0] is side without the first atom in the edge and fragments[1] is reverse
            ...
            # Identity what fragment to rotate
            # If base_idx is in fragments[1] or the first fragment is smaller 
            # than the second one rotate the first. If not, rotate the fragments[0]
            if (base_idx is not None and base_idx in fragments[1]) or (
                ...
                # Mark if at least one endpoint is in a ring and check directionality of a ring endpoint
    ...
    # Build torsion dependency graph
    for edge_idx, atom_idx in fragment_index:
        ...
        for rotated_edge in rotated_edges:
            ...
            # Check if rotated edge is also a torsion edge as peripheral torsions depend on interior
    ...
    # Sort topologically, by torsion position and by atom index
    ...
    # Detect 4-valent non-ring atoms for special angle constraints
    angle_2_index = []
...
def align_sidechains_to_backbone_torch(
    ...
    ''' Align sidechains of the apo structure to match backbone frames of holo per residue, using a local N-CA-C frame '''
...
def compute_nearby_atom_mask(
    ...
    if len(lens_receptors) == 0:
        raise ValueError("Cannot compute nearby atom mask for an empty receptor.")

    atom_grid, x, y = to_atom_grid_torch(atom_pos, lens_receptors)

    if isinstance(ligand_atoms, np.ndarray):
        ligand_atoms = torch.from_numpy(ligand_atoms)
    ligand_atoms = torch.as_tensor(
        ligand_atoms,
    ...
    nearby_residues = minimum_distance < nearby_residues_atomic_radius

    if torch.sum(nearby_residues) < nearby_residues_atomic_min:
        k = min(nearby_residues_atomic_min, int(minimum_distance.numel()))
        nearby_residues = torch.zeros_like(nearby_residues)
        if k > 0:
            _, closest_residues = torch.topk(minimum_distance, k=k, largest=False)
            nearby_residues[closest_residues] = True

    nearby_atoms = torch.zeros(
        (atom_grid.shape[0], atom_grid.shape[1]),
        dtype=torch.bool,
        device=atom_grid.device,
    )
    nearby_atoms[nearby_residues] = True
    nearby_atoms = nearby_atoms[x, y]
    return nearby_atoms, nearby_residues
