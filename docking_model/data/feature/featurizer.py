...
from docking_model.data.feature.molecule import (
    get_lig_graph_with_matching,
    get_posebusters_edge_index,
)
from docking_model.data.conformers.protein import sidechain_conformer_matching
from docking_model.data.feature.protein import (
    get_fullrec_graph,
    construct_protein_edge_index,
)
from docking_model.data.feature.helpers import align_sidechains_to_backbone_torch
...
class Featurizer:
    ...
    def featurize_ligand(self, complex_inputs):
        ''' Build ligand graph from a set of RDKit ligands '''
        ...
        # CAUTION Ligand is overriden inside loop such that only the last one is returned
        return complex_graph

    def featurize_protein(self, complex_graph, complex_inputs):
        ''' Build apo / holo protein graph, align apo to holo, match side chain conformations '''
        ...
        try:
            ...
            if holo_rec_struct is not None:
                if self.cfg.flexible_backbone or self.cfg.flexible_sidechains:
                    ...
                    complex_graph["atom"].orig_aligned_apo_pos = aligned_apo_rec_pos.clone()
