...
from docking_model.data.parse.molecule import read_mols, read_single_mol, resolve_ligand_path
from docking_model.data.parse.protein import parse_pdb_from_path as parse_pdb_pmd
from docking_model.data.conformers.molecule import generate_conformer
...
class ComplexParser:
    ...
    def parse_ligand(self, complex_dict):
        ...
        ligand_input = complex_dict.get("ligand_input", None)
        ...
        if ligand_description == "filename":
            try:
                if ligand_input is not None:
                    ...
                    lig = read_single_mol(ligand_path, remove_hs=False)
                    ligs = [lig] if lig is not None else []
                else:
